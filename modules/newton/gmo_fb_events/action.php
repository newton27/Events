...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (isset($_POST['mode']) && $_POST['mode'] === 'save_token') {
        $sPageToken = trim(isset($_POST['page_token']) ? (string)$_POST['page_token'] : '');
        if ($sPageToken === '') {
...
<?php if (!$bTokenConfigured): ?><form class="card" method="post">
<input type="hidden" name="csrf" value="<?=gmo_h($sCsrfToken)?>">
<label for="page_token"><strong>Facebook Page access token</strong></label>
<p class="muted">Paste the Page access token generated by Meta. It will be stored in UNA settings and will not be displayed again.</p>
<input id="page_token" name="page_token" type="password" autocomplete="new-password" required style="width:100%;padding:12px;border:1px solid #b9c1ce;border-radius:8px;box-sizing:border-box">
<button name="mode" value="save_token">Save Page access token</button></form><?php endif; ?>
